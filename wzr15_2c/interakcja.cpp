...
bool czy_test_pred = false;          // testowanie algorytmu predykcji bez udzia³u cz³owieka, by mo¿na by³o porównaæ ró¿ne algorytmy przy tym samym scenariuszu zdarzeñ

FILE *f = fopen("w_plik.txt","w");    // plik do zapisu informacji testowych
...
void PoczatekInterakcji()
{
  ...
  multi_reciv = new multicast_net("224.10.10.10",10001);      // obiekt do odbioru ramek sieciowych
  multi_send = new multicast_net("224.10.10.10",10001);       // obiekt do wysy³ania ramek
...
void Cykl_WS()
{
  ...
  kat = (fabs(kat-2*M_PI) ? kat > M_PI : kat);
  sum_roznic_kat += kat;
  ...
  for (int i=0;i<iLiczbaCudzychOb;i++)
  {
	  ...
	  CudzeObiekty[i]->qOrient = CudzeObiekty[i]->qOrient * AsixToQuat(CudzeObiekty[i]->wV_kat *fDt, (CudzeObiekty[i]->wV_kat *fDt).dlugosc());
  } 
}
